...
class DatabaseService {
  ...
  Future<List<String>> getPrograms() async {
    ...
    final rows = await db.rawQuery(
        "SELECT program, MIN(created_at) AS first_added FROM $_tableName WHERE program IS NOT NULL AND program != '' GROUP BY program ORDER BY first_added ASC, program ASC");
    return rows.map((r) => r['program'].toString()).toList();
  }

  // ─────────────────────────────────────────────
  // ★ v11.01: إعادة تسمية برنامج (يحدّث كل السجلات)
  // ─────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────
  // ★ v11.01: التحقق من وجود مستفيد بنفس المفتاح
  // ─────────────────────────────────────────────
  Future<bool> beneficiaryExists({
    ...
    final existing = await findBeneficiaryByKey(
